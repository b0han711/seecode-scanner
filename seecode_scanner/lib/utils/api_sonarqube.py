...
class SonarAPIHandler(object):
    ...
    def __send_data(self, url, data=None, method='POST'):
        ...
        while True:
            try:
                ...
                self.logger.debug("URL: [{0}] Headers: [{1}] Body: [{2}]".format(resp.request.url, resp.request.headers, data))
                if resp.status_code == 200:
                    ...
                    msg = 'status_code: {0}, reason: {1}'.format(resp.status_code, resp.reason or resp.content.decode('utf-8'))
    ...
    def get_ce_component(self, component_id=None, component_key=None):
        """
        :param component_id:
        :param component_key:
        :return:
        """
        if not any((component_id, component_key)):
            raise MissingImportantScanParameters("Missing 'component_id, component_key' parameters.")
        api_url = '{0}api/ce/component'.format(self.sonar_server)
        data = {}
        if component_id:
            data['componentId'] = component_id
        if component_key:
            data['componentKey'] = component_key
        result = None
        resp = self.__send_data(url=api_url, data=data, method='GET')
        if resp:
            result = resp
